...
11. Interpret the execution tape causally. `BUYING_CONFIRMED` and `SELLING_CONFIRMED` mean taker aggression achieved matching price acceptance. `BUYERS_ABSORBED`, `SELLERS_ABSORBED`, exhaustion, or aggression without price progress are contradictions or warnings, not directional confirmation. Partial flow may inform monitoring, but publication requires qualified evidence from both Binance and Bybit without cross-venue disagreement.
