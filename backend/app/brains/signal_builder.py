...
def build_ai_driven_trade_setup(
    ...
    if decision not in {"BUY_WATCH", "SELL_WATCH"}:
        return {
            "status": "NO_TRADE",
            "reason": f"CIO decision is {decision}. No active trade setup.",
            "symbol": symbol,
            "timeframe": timeframe,
        }

    side = "LONG" if decision == "BUY_WATCH" else "SHORT"
    ...
    # Models are not permitted to invent execution levels. The current market
    # reference and ATR are the only inputs accepted by this layer.
    entry = last_price
    if entry <= 0:
        return {
            "status": "NO_TRADE",
    ...
    allocation = ((cio_result.get("institutional_dossier") or {}).get("risk_committee") or {}).get("allocation_ceiling") or {}
    committee_risk_pct = max(_finite_number(allocation.get("risk_fraction")) * 100.0, 0.0)
    risk_pct = min(configured_risk_pct, committee_risk_pct) if committee_risk_pct > 0 else 0.0
    ...
    allocation_tier = (((cio_result.get("institutional_dossier") or {}).get("risk_committee") or {}).get("allocation_tier", "RESEARCH_ONLY"))
    ...
    return {
        ...
        "grade": grade,
        "setup_type": "institutional_committee_allocation",
        ...
        "entry": {
            "mode": "measured_market_reference",
